...
const getSalesReport = async (period, options = {}) => {
  ...
  // Validate period — defense-in-depth: controller validates too but this guards direct service calls
  const validPeriods = { daily: 'day', monthly: 'month', yearly: 'year' };
  const dateTrunc = validPeriods[period] || 'day';

  // Normalize end_date to end of day (23:59:59.999) so the full day is included
  const normalizedEndDate = end_date ? new Date(new Date(end_date).setHours(23, 59, 59, 999)).toISOString() : null;
...
const getProfitReport = async (period, options = {}) => {
  ...
  // Validate period — defense-in-depth
  const validPeriods = { daily: 'day', monthly: 'month', yearly: 'year' };
  const dateTrunc = validPeriods[period] || 'day';
