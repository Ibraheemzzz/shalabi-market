...
const getCategoryTree = async (req, res) => {
  try {
    ...
    logger.error('Get category tree error:', { error: error.message, stack: error.stack });
...
const getAllCategories = async (req, res) => {
  try {
    ...
    logger.error('Get all categories error:', { error: error.message, stack: error.stack });
    return serverErrorResponse(res, 'Failed to get categories');
  }
};
...
const getCategoryById = async (req, res) => {
  try {
    ...
    logger.error('Get category error:', { error: error.message, stack: error.stack });
...
const updateCategory = async (req, res) => {
  try {
    ...
    if (error.message === 'At least one field (name or parent_id) must be provided') {
      return errorResponse(res, error.message, 400);
    }
