...
const createReview = async (user_id, product_id, reviewData) => {
  ...
  // Create review and recalculate stats inside a transaction
  // NOTE: duplicate check is inside the transaction to prevent race conditions
  return await prisma.$transaction(async (tx) => {
    // Check if user already reviewed this product (inside transaction)
    ...
    // Recalculate product stats inside the same transaction
    const stats = await tx.productReview.aggregate({
      where: { product_id, is_hidden: false, is_approved: true },
      _avg: { rating: true },
      _count: { review_id: true }
    });
...
const updateReview = async (review_id, user_id, updateData) => {
  ...
  // Update review and recalculate stats inside a transaction
  return await prisma.$transaction(async (tx) => {
    ...
    // Recalculate product stats inside the same transaction
    const stats = await tx.productReview.aggregate({
      where: { product_id: review.product_id, is_hidden: false, is_approved: true },
      _avg: { rating: true },
      _count: { review_id: true }
    });
...
const deleteReview = async (review_id, user_id) => {
  ...
  // Delete review and recalculate stats inside a transaction
  return await prisma.$transaction(async (tx) => {
...
const toggleReviewVisibility = async (review_id) => {
  ...
  // Update visibility and recalculate stats inside a transaction
  return await prisma.$transaction(async (tx) => {
    const updatedReview = await tx.productReview.update({
      where: { review_id },
