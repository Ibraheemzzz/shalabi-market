...
const createProduct = async (req, res) => {
  try {
    ...
    if (stock_quantity !== undefined && parseFloat(stock_quantity) < 0) {
...
const adjustStock = async (req, res) => {
  try {
    ...
    // Reject zero — meaningless operation
    if (parseFloat(quantity_change) === 0) {
    ...
    logger.error('Adjust stock error:', { error: error.message, stack: error.stack });
...
const getStockHistory = async (req, res) => {
  try {
    ...
    logger.error('Get stock history error:', { error: error.message, stack: error.stack });
...
const getAllProductsAdmin = async (req, res) => {
  try {
    ...
    logger.error('Get all products admin error:', { error: error.message, stack: error.stack });
    return serverErrorResponse(res, 'Failed to get products');
  }
};
