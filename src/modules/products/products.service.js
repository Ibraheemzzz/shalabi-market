...
const getProducts = async (options) => {
  const { search, category_id, sale_type, sort, order } = options;
  ...
  // Whitelist allowed sort fields to prevent exposing sensitive columns (e.g. cost_price)
  const allowedSortFields = ['created_at', 'name', 'price', 'average_rating', 'reviews_count', 'stock_quantity'];
  ...
  const products = await prisma.product.findMany({
    ...
    select: {
      ...
      description: true,
      price: true,
      // cost_price intentionally excluded from public endpoint
      sale_type: true,
      stock_quantity: true,
      image_url: true,
      average_rating: true,
  ...
  return buildPaginatedResponse(transformedProducts, totalItems, safePage, safeLimit);
};
...
const adjustStock = async (product_id, stockData) => {
  ...
  // Reject zero — no change means no transaction needed
  if (change === 0) {
    throw new Error('quantity_change cannot be zero');
  }

  // Run inside transaction to avoid race condition:
  // Read, validate, and write all happen atomically
  return await prisma.$transaction(async (tx) => {
    // Re-read stock inside the transaction for accurate value
    ...
    // Use atomic set inside the transaction (value already locked by tx read)
...
const getAllProductsAdmin = async (options) => {
  const { search, category_id, sale_type, is_active, sort, order } = options;
  const { skip, take, page: safePage, limit: safeLimit } = safePaginate(options.page, options.limit);
  ...
  // Whitelist allowed sort fields (admin can also sort by is_active)
  const allowedSortFields = ['created_at', 'name', 'price', 'average_rating', 'reviews_count', 'stock_quantity', 'is_active'];
  const sortField = allowedSortFields.includes(sort) ? sort : 'created_at';
  const sortOrder = ['asc', 'desc'].includes((order || '').toLowerCase()) ? order.toLowerCase() : 'desc';
