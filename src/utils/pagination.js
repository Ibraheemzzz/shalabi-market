 ...
 */
const buildSearchFilter = (searchTerm, searchFields, startParamIndex = 1) => {
  ...
  const likeConditions = searchFields.map((field, i) => `${field} ILIKE $${startParamIndex + i}`);
...
/**
 * Safe pagination calculation — guards against page=0, negative, or non-numeric values
 * Use this instead of computing skip/take manually in every service.
 * @param {*} page - Raw page value from query
 * @param {*} limit - Raw limit value from query
 * @param {number} defaultLimit
 * @param {number} maxLimit
 * @returns {{ skip: number, take: number, page: number, limit: number }}
 */
const safePaginate = (page, limit, defaultLimit = 20, maxLimit = 100) => {
  let p = parseInt(page) || 1;
  let l = parseInt(limit) || defaultLimit;
  if (p < 1) p = 1;
